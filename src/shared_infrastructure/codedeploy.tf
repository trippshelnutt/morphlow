...
resource "aws_codedeploy_app" "app_codedeploy_app" {
  ...
  compute_platform = "ECS"
  name             = "${each.key}_app_code_deploy_app"
}

resource "aws_codedeploy_deployment_group" "app_codedeploy_group" {
  for_each = local.environments

  app_name               = aws_codedeploy_app.app_codedeploy_app[each.key].name
  deployment_config_name = "CodeDeployDefault.ECSAllAtOnce"
  deployment_group_name  = "${each.key}_app_codedeploy_group"
  service_role_arn       = data.aws_iam_role.morphlow_codedeploy_role.arn
  ...
  ecs_service {
    ...
    service_name = aws_ecs_service.app_service[each.key].name
  }
  ...
  load_balancer_info {
    target_group_pair_info {
      ...
      target_group {
        name = aws_lb_target_group.app_blue_target_group[each.key].name
      }

      target_group {
        name = aws_lb_target_group.app_green_target_group[each.key].name
      }
    }
  }
}